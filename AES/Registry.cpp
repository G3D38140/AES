...
BOOL CRegistry::VerifyKey (HKEY hKeyRoot, LPCTSTR pszPath)
{
	...
	if(ReturnValue == ERROR_SUCCESS) return TRUE;
...
{
	ASSERT(m_hKey);
	LONG lReturn = RegQueryValueEx(m_hKey, pszValue, NULL,NULL, NULL, NULL);

	m_Info.lMessage = lReturn;
	m_Info.dwSize = 0L;
	m_Info.dwType = 0L;

	if(lReturn == ERROR_SUCCESS) return TRUE;
...
BOOL CRegistry::CreateKey (HKEY hKeyRoot, LPCTSTR pszPath)
{
	...
	LONG ReturnValue = RegCreateKeyEx(hKeyRoot, pszPath, 0L, NULL, REG_OPTION_NON_VOLATILE, KEY_ALL_ACCESS, NULL, &m_hKey, &dw);
...
BOOL CRegistry::Write (LPCTSTR pszKey, LPCTSTR pszData)
{
	...
	CString tmmp(pszData); 
	int lg = (tmmp.GetLength() + 1) * sizeof(TCHAR);

	LONG ReturnValue = RegSetValueEx(m_hKey, pszKey, 0L, REG_SZ, (const BYTE *)(LPCTSTR)pszData, lg); // (char*)  REG_EXPAND_SZ

	m_Info.lMessage = ReturnValue;
	m_Info.dwSize = strlen((char*)pszData) + 1;
...
BOOL CRegistry::Write (LPCTSTR pszKey, CStringList& scStringList)
{
	...
	LONG lReturn = RegSetValueEx(m_hKey, pszKey, 0, REG_BINARY,file.Detach(), dwLen);
...
BOOL CRegistry::Read(LPCTSTR pszKey, int& iVal)
{
	...
	LONG lReturn = RegQueryValueEx (m_hKey,  pszKey, NULL,&dwType, (BYTE *) &dwDest, &dwSize);
...
BOOL CRegistry::Read (LPCTSTR pszKey, DWORD& dwVal)
{
	...
	LONG lReturn = RegQueryValueEx (m_hKey,  pszKey, NULL, &dwType, (BYTE *) &dwDest, &dwSize);// (LPSTR)
...
BOOL CRegistry::Read (LPCTSTR pszKey, CString& sVal)
{
	...
	LONG lReturn = RegQueryValueEx (m_hKey,(LPCWSTR) pszKey, NULL,&dwType, (BYTE *) szString, &dwSize);
...
BOOL CRegistry::Read (LPCTSTR pszKey, CStringList& scStringList)
{
	...
	LONG lReturn = RegQueryValueEx(m_hKey, (LPCWSTR) pszKey, NULL, &dwType, byData, &dwData);
